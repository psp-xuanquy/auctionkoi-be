...
{
    public class CheckAscendingBidAuctionHandler : BaseAuctionHandler, IRequestHandler<CheckAscendingBidAuctionCommand, bool>
    {
        private readonly IEmailService _emailService;

        public CheckAscendingBidAuctionHandler(ICurrentUserService currentUserService, IKoiRepository koiRepository, IBidRepository bidRepository, IAutoBidRepository autoBidRepository, IUserRepository userRepository, IEmailService emailService)
            : base(koiRepository, bidRepository, autoBidRepository, userRepository, currentUserService)
        {
            _emailService = emailService;
        }

        public async Task<bool> Handle(CheckAscendingBidAuctionCommand request, CancellationToken cancellationToken)
        {
